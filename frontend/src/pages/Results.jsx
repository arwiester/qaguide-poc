...
const FRAMEWORKS = {
  javascript: ['Playwright (UI)', 'Cypress (UI)', 'Playwright API', 'Postman/Newman', 'k6 (Load)'],
  typescript: ['Playwright (UI)', 'Cypress (UI)', 'Playwright API', 'Postman/Newman', 'k6 (Load)'],
  csharp:     ['Playwright .NET', 'Selenium WebDriver', 'RestSharp (API)'],
  python:     ['Playwright Python', 'Selenium WebDriver', 'Requests (API)'],
  java:       ['Selenium WebDriver Java', 'RestAssured (API)', 'Playwright Java'],
};
...
function ExecutionPanel({ code, language, framework }) {
  ...
  return (
    <div className="mt-4 space-y-3">
      ...
      {logs.length > 0 && (
        <div className="bg-gray-900 rounded p-3 max-h-80 overflow-y-auto font-mono text-xs space-y-0.5">
          {logs.map((entry, i) => (
            <div key={i} className={
        ...
        <>
          ...
          {userLevel === 'tester' && (
            <p className="text-sm text-gray-500 bg-gray-50 border border-gray-200 rounded px-4 py-3">
              💡 Some of these workflows could be tested at the API level — faster and more reliable than UI testing. Select Engineer level to unlock code generation.
            </p>
          )}
